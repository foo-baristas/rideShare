...
var express = require('express'),
    ...
    checkit = require('../models/resource');
...
function exists(name) {
    ...
    return knex.select('*').from('users').where({
        name_first: nameArray[0],
        name_last: nameArray[1]
    });
}
...
// checkit.validate(post).then(function(validated) {
    knex('users').update({
            username: post.username,
            password: hash,
            name_first: post.name_first,
        ...
        .then(function(id) {
            res.redirect('/user/' + id);
          //UNCOMMENT HERE FOR CHECKIT.VALIDATE
          // });
        }).catch(function(err) {
            console.error(err);
            res.send(err);
